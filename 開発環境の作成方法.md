...
6. **pyenvでインストールできるpythonのバージョンを調べる**

   ```bash
   pyenv install --list | grep -E "^\s*[0-9]+\.[0-9]+\.[0-9]+$"
   ```

7. **pyenvでpythonをインストールする前に必要なライブラリのインストール**
   ```bash
...
8. **pyenvでpythonのインストール**

   ```bash
   pyenv install 3.12.7
   ```
9. **通常使うpythonのバージョンを指定する**

   pyenvでインストールされているバージョンの一覧を表示するコマンド
   ```bash
   pyenv versions
   ```
   グローバルの場合
   ...
   python --version
   ```
   バージョンが変わらない場合は、シェルの再読み込みを行ってみてください
   ```bash
   source ~/.bashrc
   ```
...
## グラフを表示させるためのライブラリインストール

これを実行するとこれまでpipでインストールしたライブラリが削除されます。
...
2. 6のAdvanced OptionsでA6のWaylandを選択

3. W1のX11を選択して了解を押してX11へ設定変更を実施

4. Rebootを促されるのでリブートを実行

5. Reboot後にRaspberryマークから設定→Raspberry Piの設定でInterfaceのVNCを有効に（もし有効化がすでにされていた場合は無効化し再度有効化してください）

すると画面右上にVNCが起動しアイコンが表示されます。

### パスワードについて
VNC側のパスワードを設定することで接続できます。

1. OptionsのAuthenticationを`VNC password`
